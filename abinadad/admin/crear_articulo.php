...
<?php 
     ...
     if (isset($_POST['crearArticulo'])) {
        ...
        $titulo = $_POST["titulo"];
        $texto = $_POST["texto"];
        $idPublicador = $_POST["idPublicador"];
        ...
        if ($_FILES["imagen"]["error"] > 0) {
            ...
            if (empty($titulo) || $titulo == '' || empty($texto) || $texto == '') {
                ...
                if ($articulo->crear($titulo, $newImageName, $texto, $idPublicador)) {
    ...
    <div class="row">
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            <?php foreach($resultado as $usuario) : ?>
                <?php if($usuario->usuario_email == $_SESSION['email']) : ?>
                    <input type="hidden" class="form-control" name="idPublicador" id="idPublicador" value="<?php echo  $usuario->usuario_id ?>">
                <?php endif; ?>
            <?php endforeach; ?>
